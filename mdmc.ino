 ...
 *       [si5351 SDA] | [X]A4/SDA               D5[X]~| Pad Port Pin 3
 *       [si5351 SCL] | [X]A5/SCL               D4[X] | Pad Port Pin 4
...
#define JAP_COMBO MD_BTN_RIGHT
#define EUR_COMBO MD_BTN_UP
...
// This will be handy
  ...
  #define MODE_LED_EUR_COLOR {0x00, 0x00, 0xFF}  // Blue
...
//      debugln (F("Mode unchanged, not saving"));
    ...
    #ifdef ENABLE_MODE_LED_RGB // agregado 
...
void setup () {
  ...
  // the rest later.
  clockgen.begin();
//  noInterrupts (); // talvez no se necesite?
...
//  debugln (current_mode);
//  interrupts (); // talvez no se necesite?
  set_mode (current_mode, false); // tiene que estar fuera del bloque nointerrupts para poder configurar clockgen
//  dstart (57600);
...
inline void setup_pad () {
  ...
  pinMode (6, INPUT);
  pinMode (7, INPUT);
  // The SIGNALING line is an output
//  ~ pinMode (12, OUTPUT);
